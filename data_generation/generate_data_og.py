...
def generate_products() -> typing.List[dict]:
    ...
    with open(
        f"data/products.csv", encoding="utf-8"
    ) as productcsv:
...
def generate_locations() -> typing.List[str]:
    location_data = []
    with open(
        f"data/world_pop.csv", encoding="utf-8"
    ) as worldcsv:
...
def main(
    ...
    # save all data to csv files locally
    logging.info("writing data to local csv files")
    ...
    for name, table_dat in list(zip(table_name, table_dat)):
        ...
        logging.info(f"writing output file to... {source_dir}/{name}.csv")
        with open(f"{source_dir}/{name}.csv", "w", encoding="utf-8") as f:
            f.write(csv_data)
